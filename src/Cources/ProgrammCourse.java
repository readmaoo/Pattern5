...
public class ProgrammCourse implements Courses {
    private boolean completed = false;
    private static final Scanner IN = new Scanner(System.in);

    @Override
    public String nameOftheCourse() { return "Program Course"; }

    @Override
    public int mainContent() {
        int quizPoints = 0;
        try (InputStream is = getClass().getResourceAsStream("/program.json")) {
            if (is == null) {
                System.out.println("Content is not found: /program.json");
                return 0;
            }
            ...
            System.out.println("error Program: " + e.getMessage());
        }
        return quizPoints;
    }
